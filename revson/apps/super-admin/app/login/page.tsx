...
import { supabaseBrowser, portalForRole, APP_URLS, Button, Card, Field, Input, toast } from "@revson/shared";
...
export default function AdminLoginPage() {
  ...
  async function submit() {
    ...
    if (profile.role !== "super_admin") { window.location.href = portalForRole(profile.role); return; }
    ...
    <main className="flex min-h-screen items-center justify-center bg-cream px-4">
      <Card className="w-full max-w-md p-8">
        <div className="mb-6 flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-coal text-sm font-bold text-white">R</span>
          <span className="font-semibold text-ink">Revson — Super Admin</span>
        </div>
